...
// 11 0.142
// 12 0.132
const square_10 = squaresObj(14, 0.111);
...
function makeFloor(num = 100) {
  var planeGeometry = new THREE.PlaneGeometry(screen.width, screen.height, 30, 30);
...
function animate() {
  ...
  if ((clicked && pushed) || avgFreq > 0) {
    ...
    makeRoughPlane(plane, upperFreq);
    changeBorderBox(square_6, avgFreq, 70);
    changeBorderBox(square_10, avgFreq, 70, true);
  }
  controls.update();
...
function makeRoughPlane(mesh, fr) {
  mesh.geometry.vertices.forEach(function (vertex, i) {
    var amp = 0.8;
    var time = Date.now();
    var distance =
      (noise.noise2D(vertex.x + time * 0.0003, vertex.y + time * 0.0001) + 0) * fr * amp;
    vertex.z = distance;
  });
  mesh.geometry.verticesNeedUpdate = true;
  mesh.geometry.normalsNeedUpdate = true;
